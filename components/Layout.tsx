...
import { LayoutDashboard, IndianRupee, FileText, Settings, Wallet, Building, CalendarDays, Users, Cloud, CloudOff, RefreshCw } from 'lucide-react';
...
const Layout: React.FC<LayoutProps> = ({ children, currentView, setView, syncStatus = 'IDLE' }) => {
  const navItems = [
    { 
      id: 'DASHBOARD', 
  ...
  return (
    <div className="flex flex-col h-screen bg-slate-50 dark:bg-slate-950 transition-colors duration-200">
      <header className="bg-white/80 dark:bg-slate-900/80 backdrop-blur-md border-b border-slate-200 dark:border-slate-800 px-4 py-3 sticky top-0 z-20 flex justify-between items-center shadow-sm">
        <div className="w-8 flex items-center justify-center" title={`Sync Status: ${syncStatus}`}>
          {syncStatus === 'SYNCING' && <RefreshCw className="animate-spin text-blue-500" size={18} />}
          {syncStatus === 'SYNCED' && <Cloud className="text-green-500" size={18} />}
      ...
      <nav className="fixed bottom-0 w-full bg-white dark:bg-slate-900 border-t border-slate-100 dark:border-slate-800 pb-safe z-30 shadow-[0_-8px_30px_rgba(0,0,0,0.04)] dark:shadow-none transition-colors duration-200 overflow-x-auto no-scrollbar">
          ...
          {navItems.map((item) => {
            ...
            return (
              <button
                ...
                className={`group flex flex-col items-center justify-center w-16 py-1.5 rounded-xl transition-all duration-300 ease-in-out relative overflow-hidden flex-shrink-0 ${
                  isActive 
              ...
              >
                ...
                <span className={`text-[9px] font-bold mt-0.5 truncate max-w-full px-0.5 transition-all duration-300 ${isActive ? 'opacity-100 translate-y-0' : 'opacity-70 group-hover:opacity-100'}`}>
                  {item.label}
                </span>
