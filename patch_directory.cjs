...
code = code.replace(
  /'DAILY' \| 'UNPAID' \| 'OUTSTANDING' \| 'AUDIT' \| 'BILLING' \| 'BANK_TRANSFER' \| 'DIRECTORY'/,
  "'DAILY' | 'UNPAID' | 'OUTSTANDING' | 'AUDIT' | 'BILLING' | 'BANK_TRANSFER' | 'OWNERS' | 'TENANTS'"
);
...
code = code.replace(
  /<button onClick=\{\(\) => setActiveTab\('DIRECTORY'\)\}[\s\S]*?Directory\s*<\/button>/,
  newTabs
);
