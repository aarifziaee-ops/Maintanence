...
async function run() {
  ...
  if (docSnap.exists()) {
    state = docSnap.data();
    console.log(`Loaded state with ${state.transactions.length} transactions.`);
  } else {
...
`;
  ...
  const existingTxIds = new Set(state.transactions.map(t => String(t.receiptNo)));
  
  let addedCount = 0;
  for (const line of lines) {
    const [dateStr, flatNumber, amount, receiptNo, ownerName] = line.split(',');
    if (!existingTxIds.has(String(receiptNo))) {
  ...
  // also add income financial records for these
  const existingFinIds = new Set(state.financialRecords.filter(f => f.type === 'INCOME').map(f => f.description));
  ...
  for (const line of lines) {
    ...
    const desc = `Maintenance - ${flatNumber} (Receipt #${receiptNo})`;
    if (!existingFinIds.has(desc)) {
